...
const Features = () => {
    return (
        <section className="text-center">
            ...
            <p className="bg-linear-to-b from-zinc-50 to-zinc-300 text-transparent bg-clip-text text-2xl">Stay Consistent, compete with friends and <span className="text-(--color-heading-primary) text-semibold  ">Grow</span></p>

            <div className="relative grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 rounded-4xl mt-16 text-white">
                ...
                {features.map(feature => (
                    <div key={feature.title} className="rounded-3xl shadow-xl p-8 bg-[#0b1d0da1] text-left space-y-4  transform transition-transform duration-300 hover:-translate-y-2 ">
                        ...
                        <p className="text-neutral-100">{feature.text}</p>
                    </div>
                ))}
            </div>
        </section>
